...
class EncryptTestCase(unittest.TestCase):
    ...
    def test_error_lineA(self):
        self.assertEqual(calculator(""), 'input error in calculator')
    ...
    def test_too_many_numbers(self):
        self.assertEqual(calculator('2 3 4 54 +1'), 'input error line B: calculator')
    ...
    # Tests for 'HW4' 
    def test_simpleParens(self):
        self.assertEqual(calculator('2*(4+1)'), 10)
    def test_complexParens(self):
        self.assertEqual(calculator('2*(4+1)+3*(6-(4+1*5-5))'), 16)

    def test_simpleNegation(self):
        self.assertEqual(calculator('2* -4'), -8)
    def test_simpleNegation2(self):
        self.assertEqual(calculator('2+ -8--15'), 9)

    def test_exampleTest1(self):
        self.assertEqual(calculator('3*(10 - 2*3)'), 12)
    def test_exampleTest2(self):
        self.assertEqual(calculator("- 2 / (- 4)*(3-2*(4-2^3))+3".strip()), 8.5)
    def test_exampleTest4(self):
        self.assertEqual(calculator("2*(4+2*(5-3^2)+1)+4"), -2)
    def test_exampleTest5(self):
        self.assertEqual(calculator("-(-2)*10 - 3*(2 - 3*2) "), 32)
    def test_exampleTest6(self):
        self.assertEqual(calculator("-(-2)*10 - 3*(2 - 3*2)) "), "error: mismatched parentheses")
    def test_exampleTest7(self):
        self.assertEqual(calculator("-( - 2)*10 - 3*/(2 - 3*2) "), "error: no number between operators")
